...
import com.telek.entities.others.*;
...
import com.telek.world.*;
import com.telek.world.OthersFactory;

import java.util.*;
import static com.telek.utils.Consts.*;
...
public class GameScreen extends ScreenAdapter implements TScreen {
    ...
    private void loadCurrentLevel() {
        clearLasers();
        damageBalls = com.telek.world.EnemyFactory.getDamageBalls(this.game.assetSorter, world, player.getCurrentLevel());
        trapTiles = com.telek.world.EnemyFactory.getTrapTiles(this.game.assetSorter, player.getCurrentLevel());
        timedTiles = com.telek.world.EnemyFactory.getTimedTiles(this.game.assetSorter, player.getCurrentLevel());
        sentries = EnemyFactory.getSentries(this.game.assetSorter, world, player.getCurrentLevel());
    }
    ...
    public void update(float delta) {
        player.move();
        for(DamageBall ball : damageBalls) ball.update(delta);
        for(TrapTile trapTile : trapTiles) trapTile.update(lasers, player, delta);
        for(TimedTile timedTile : timedTiles) timedTile.update(lasers, player, delta);
        for(Laser laser : lasers) laser.update(lasers, player, delta);
        for(Iterator<Laser> iterator = lasers.iterator(); iterator.hasNext(); ){
            temp = iterator.next();
            if(temp.shouldBeRemoved()) iterator.remove();
        }
        for(Sentry sentry : sentries) sentry.update(lasers, player, delta);
    ...
    private void updateCam() {
        ...
        pos.x = this.game.camera.position.x + (player.getSprite().getX() - this.game.camera.position.x) * LERP;
        pos.y = this.game.camera.position.y + (player.getSprite().getY() - this.game.camera.position.y) * LERP;
    ...
    public void render(float delta) {
        ...
        if(player.canMove()) {
            for(TimedTile timedTile : timedTiles) timedTile.draw(this.game.batch);
            player.draw(this.game.batch);
            for(TrapTile trapTile : trapTiles) trapTile.draw(this.game.batch);
            for(DamageBall ball : damageBalls) ball.draw(this.game.batch);
            for(Sentry sentry : sentries) sentry.draw(this.game.batch);
            for(Laser laser : lasers) laser.draw(this.game.batch);
        }
        else {
    ...
    private void checkIfGameHasEnded(Player player) {
        ...
        this.game.setScreen(game.screenSorter.getScreen("endingScreen"));
    }
